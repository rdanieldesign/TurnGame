...
var fight = function(){
  ...
  for (var i in levelEnemies){
    ...
    $(document).keypress(function(event){
      event.preventDefault();
      if(isTargeted){
        $(enemyID).css({'background-position': '-636px 0', 'width': '340px'});
        setTimeout(function(){
          $(enemyID).css({'background-position': '0 0', 'width': '300px'});
        }, 250);
        targHealth -= (Math.random() * 100);
